...
include_once 'furworld_datanetwork.php';
...
if (isset($_POST['attemptreset'])) {
	...
	if ($captcha_success->success == false){
		header("Location: https://dev.sda.one/projects/fwio/account/reset.php?email=$email&resetcode=$resethash&e=captchafail");
		exit();
		...
		if ( $result->num_rows == 0 ){
			...
			header("Location: https://dev.sda.one/projects/fwio/account/login.php?e=resetfailnoaccount");
			exit();
		} else{
			$pword = $_POST['pword'];
			...
			if ($pword == $pword_conf){
				...
				$result = mysqli_query($con, $sql);
				header("Location: https://dev.sda.one/projects/fwio/account/login.php?e=resetcomplete");
				exit();
			
			}else{
				header("Location: https://dev.sda.one/projects/fwio/account/reset.php?email=$email&resetcode=$resethash&e=passwordmismatch");
				exit();
		}
	}	
}
