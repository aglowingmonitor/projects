...
if (isset($_POST['submitreset'])) {
    ...
    if (empty($email)){
        ...
        if ($resultCheck == 0){
			...
			if (!$q){
				echo "<h1>FAM THE QUERY FAILED CHECK YO SHIT</h1>";
			}
						
            //Send email off oWo
            $to = $email;
            $subject = "Furworld - Password Reset Request";
            $message_body = "Someone has requested an account password reset using your email address.
			
			To reset your password, click the link below.
			https://dev.sda.one/projects/fwio/account/reset_password.php?email=".$email.'&resetcode='.$resetCode."
